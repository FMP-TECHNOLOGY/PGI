...
namespace Utils.Helpers
{
    ...
    public class FechaLetra
    {
        public static string? GetDayName(int d)
        {
            ...
            if (d > 10 && d < 20)
            {
                return "DIECI" + GetDayName(d - 10);
            }
            if (d == 20) return "VEINTE";
            if (d > 20 && d < 30)
            {
                return "VEINTI Y " + GetDayName(d - 10);
            }
            if (d == 30) return "TREINTA";
            if (d == 31) return "TREINTA Y UNO";
