...
namespace API_PGI.Controllers.TipoRiesgos
{
    ...
    public class TipoRiesgoController : ControllerBase
    {
        ...
        public TipoRiesgoController(ITipoRiesgo TipoRiesgo, IAuth auth)
        {
            _TipoRiesgo = TipoRiesgo;
            _Auth = auth;
        }

        [HttpPost]
        public IActionResult post([FromBody] TipoRiesgo entity)
        {
            try
            {
        ...
        [HttpPut]
        public IActionResult Put([FromBody] TipoRiesgo entity)
        {
            try
            {
        ...
        public IActionResult GetAll([FromQuery] GridifyQuery gridifyQuery)
        {
            try
            {
                var builder = new QueryBuilder<TipoRiesgo>()
                             .AddQuery(gridifyQuery)
                             .AddCondition($"{nameof(TipoRiesgo.CompaniaId)}={_Auth.CurrentUser?.CompaniaId}")
                ;
        ...
        public IActionResult GetAll(string id)
        {
            try
            {
                var valor = _TipoRiesgo.Find(x => x.Id == id);
