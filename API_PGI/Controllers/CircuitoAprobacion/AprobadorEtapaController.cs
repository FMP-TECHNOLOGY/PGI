...
namespace API_PGI.Controllers.CircuitoAprobacion
{
    ...
    [ApiController]
    [JwtAuthorize]
    public class AprobadorEtapaController : ControllerBase
    {
        private readonly IAprobadorEtapa _Repo;
        private IAuth _Auth { get; }

        public AprobadorEtapaController(IAprobadorEtapa BaseSystemData, IAuth auth)
        {
        ...
        [HttpPost]
        public IActionResult Post([FromBody] AprobadorEtapa entity)
        {
            try
            {
        ...
        [HttpPut]
        public IActionResult Put([FromBody] AprobadorEtapa entity)
        {
            try
            {
        ...
        public IActionResult GetAll([FromQuery] GridifyQuery gridifyQuery)
        {
            try
            {
                var builder = new QueryBuilder<AprobadorEtapa>()
                             .AddQuery(gridifyQuery)
                             //.AddCondition($"{nameof(AprobadorEtapa.CompaniaId)}={_Auth.CurrentUser?.CompaniaId}")
                             ;
