...
namespace API_PGI.Controllers.Indicadores
{
    ...
    public class IndicadoreController : ControllerBase
    {
        ...
        {
            _Indicadore = Indicadore;
            _Auth = auth;
        }

        [HttpPost]
        public IActionResult post([FromBody] Indicadore entity)
        {
            try
            {
        ...
        [HttpPut]
        public IActionResult Put([FromBody] Indicadore entity)
        {
            try
            {
        ...
        public IActionResult GetAll([FromQuery] GridifyQuery gridifyQuery)
        {
            try
            {
                var builder = new QueryBuilder<Indicadore>()
                             .AddQuery(gridifyQuery)
                             .AddCondition($"{nameof(Indicadore.CompaniaId)}={_Auth.CurrentUser?.CompaniaId}")
                ;
                if (gridifyQuery.PageSize == 0) gridifyQuery.PageSize = int.MaxValue;
                if (gridifyQuery.Page == 0) gridifyQuery.Page = 1;

                var items = _Indicadore.GetPaginated(gridifyQuery);
                return Ok(new ResponseModel()
                {
        ...
        public IActionResult GetAll(string id)
        {
            try
            {
                var valor = _Indicadore.Find(x => x.Id == id);
