...
namespace API_PGI.Controllers.Permissions
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        public PermissionController(IPermission Permission, IAuth auth)
        {
        ...
        [HttpPut]
        public IActionResult Put([FromBody] Permission entity)
        {
            try
            {
        ...
        public IActionResult GetAll([FromQuery] GridifyQuery gridifyQuery)
        {
            try
            {
                var builder = new QueryBuilder<Permission>()
                             .AddQuery(gridifyQuery)
                             .AddCondition($"{nameof(Permission.CompaniaId)}={_Auth.CurrentUser?.CompaniaId}")
                ;
                if (gridifyQuery.PageSize == 0) gridifyQuery.PageSize = int.MaxValue;
                if (gridifyQuery.Page == 0) gridifyQuery.Page = 1;

                var items = _Permission.GetPaginated(gridifyQuery);
                return Ok(new ResponseModel()
                {
        ...
        public IActionResult GetAll(string id)
        {
            try
            {
                var valor = _Permission.Find(x => x.Id == id);
