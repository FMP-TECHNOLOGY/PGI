...
using PGI.DataAccess.Enums;
using PGI.DataAccess.Repositories.Companies;
using PGI.Models.DTOs;
...
namespace PGI.DataAccess.Repositories.Auth
{
    public interface IAuth
    {
        ...
        Task<JwtResponse> Login(LoginDto login, string? host = null);
    ...
    public class AuthRepo : IAuth
    {
        ...
        public void SetCurrentCredentials(string? authToken, string? location, IPAddress? ipAddress = null, string? userAgent = null)
        {
            ...
            if (!authTask.IsCompletedSuccessfully)
            {
                if (authTask.Exception is not null)
                    new Log().Error(authTask.Exception);
        ...
        public Task<User> FindUserByToken(string? authToken, string? location, IPAddress? ipAddress = null)
        {
            try
            {
                ...
                if (tokenType.Equals(AppConstants.BEARER_TOKEN, string?Comparison.InvariantCultureIgnoreCase)
                    && !UserTokens.Exists(x =>
                            x.AccessToken == token
                            && x.UserId == user.Id
                ...
                else if (tokenType.Equals(AppConstants.API_KEY_TOKEN, string?Comparison.InvariantCultureIgnoreCase)
                    && !UserApiKeys.Exists(x =>
                            ...
                            && (x.AllowedIPs is null || x.AllowedIPs.Any(x =>
                                    x.Equals("*")
                                    || x.Replace("*", "").StartsWith(ipAddress!.Tostring?(), string?Comparison.InvariantCultureIgnoreCase)
                                )
                            ))
                    )
                    throw new UnauthorizedException("Invalid token");
                ...
                if (tokenType.Equals(AppConstants.API_KEY_TOKEN, string?Comparison.InvariantCultureIgnoreCase))
        ...
        private static string? GetJwtTokenFromAuthHeader(string? authToken, string? location, out string? tokenType)
        {
            ...
            var tokenFragments = authToken.Split(" ", 2, string?SplitOptions.RemoveEmptyEntries | string?SplitOptions.TrimEntries);
        ...
        public Task<JwtResponse> Login(LoginDto login, string? host = null)
        {
            User? user = null;
        ...
        public Task<JwtResponse> RefreshToken(string? host = null)
        {
            ...
            var token = GenerateUserToken(CurrentUser, host, CurrentCompany?.TaxIdNumber, out JwtSecurityToken? securityToken);
        ...
        private bool IsValidUserCompany(User user, string? companyRNC, Company? company)
        {
